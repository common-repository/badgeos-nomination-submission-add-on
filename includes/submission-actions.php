...
function badgeos_save_nomination_data() {
	...
	// $badgeos_nomination_content 	= sanitize_text_field($_POST['badgeos_nomination_content']);
	$badgeos_nomination_user_id 	= sanitize_text_field($_POST['badgeos_nomination_user_id']);
	$user_id 						= sanitize_text_field($_POST['user_id']);

	$badgeos_nomination_content 	=  '';
	if( isset( $_POST['badgeos_nomination_content'] ) ) {
		$badgeos_nomination_content 	= wp_kses_post( $_POST['badgeos_nomination_content'], $allowedposttags );
	}
	...
	return badgeos_create_nomination(
		...
		sprintf( '%1$s: %2$s', ucwords( get_post_type( absint( $achievement_id ) ) ), get_the_title( absint( $achievement_id ) ) ),
		$badgeos_nomination_content,
		absint( $badgeos_nomination_user_id ),
		absint( absint( $user_id ) )
	);
}
...
function badgeos_create_nomination( $achievement_id  = 0, $title = '', $content = '', $user_nominated  = 0, $user_nominating = 0 ) {
	// if ( badgeos_check_if_user_has_nomination( absint( $user_nominated ), absint( $achievement_id ) ) ) {
		...
		$submission_data = array(
			...
			'post_status'	=>	'publish',
			'post_author'	=>	absint( $user_nominating ),
...
function badgeos_add_nomination_columns( $columns = array() ) {
	...
	$column_content = array( 'content' => __( 'Content', 'badgeos' ) );
	$column_nominated_by = array( 'nominated_by' => __( 'Nominated By', 'badgeos' ) );
	$column_nominee_user = array( 'nominee_user' => __( 'Nominee', 'badgeos' ) );
	//$column_action = array( 'action' => __( 'Action', 'badgeos' ) );
	$column_status = array( 'status' => __( 'Status', 'badgeos' ) );

	$columns = array_slice( $columns, 0, 2, true ) + $column_content + array_slice( $columns, 2, NULL, true );
	$columns = array_slice( $columns, 0, 3, true ) + $column_nominated_by + array_slice( $columns, 2, NULL, true );
	$columns = array_slice( $columns, 0, 4, true ) + $column_nominee_user + array_slice( $columns, 2, NULL, true );
...
function badgeos_submission_column_action( $column = '' ) {
	...
	switch ( $column ) {
		...
		case 'content':
			add_thickbox();
			$subnom_content_id = $post->ID;
			$content = $post->post_content;

			$content_data  = '<div id="'.$subnom_content_id.'" style="display:none" >';
			$content_data .= '<p>'. html_entity_decode( wpautop( $content ), ENT_QUOTES, 'UTF-8' ) .'</p>';
			$content_data .= '</div>';

			$content_link  =  '<a title="Content" class="thickbox" href="#TB_inline?&width=800&height=550&inlineId='.$subnom_content_id .'">';
			$content_link .=  'view content </a>';
		...
		case 'nominated_by':
			...
			$user_id = ( get_post_type( $post ) == 'submission' ) ? get_post_meta( $post->ID, '_badgeos_submission_user_id', true ) : get_post_meta( $post->ID, '_badgeos_nominating_user_id', true );
...
function badgeos_process_submission_review( $post_id = 0 ) {
	...
	if( isset( $_POST['post_type'] ) ) {
		...
		$status 				= sanitize_text_field( $_POST['_badgeos_'.$post_type.'_status'] );
		$post_type_status 		= get_post_meta( $post_id, '_badgeos_'.$post_type.'_status', true );
	}
	if (
		!empty( $_POST )
		&& isset( $post_type ) // verify post_type is set
		&& in_array( $post_type, array( 'submission', 'nomination' ) ) // verify post type is submission/nomination
		...
		&& isset( $status ) // verify status is set
		&& $status != $post_type_status // if submission/nomination hasn't changed, skip it
	) {

		$badgeos_nominated_user_id 		= sanitize_text_field($_POST['_badgeos_nominated_user_id']);
		$post_author 					= sanitize_text_field($_POST['post_author']);

		// Get the achievement and user attached to this submission/nomination
		$achievement_id = get_post_meta( $post_id, '_badgeos_'.$post_type.'_achievement_id', true );
		$user_id = isset($badgeos_nominated_user_id)? $badgeos_nominated_user_id : $post_author;
...
function badgeos_save_submission_data() {
	...
	$badgeos_submission_submit 		=  '';
	if( isset( $_POST['badgeos_submission_submit'] ) ) {
		$badgeos_submission_submit 		= sanitize_text_field( $_POST['badgeos_submission_submit'] );
	}

	$badgeos_submission_draft 		=  '';
	if( isset( $_POST['badgeos_submission_draft'] ) ) {
		$badgeos_submission_draft 		= sanitize_text_field( $_POST['badgeos_submission_draft'] );
	}
	...
	$badgeos_submission_content 		=  '';
	if( isset( $_POST['badgeos_submission_content'] ) ) {
		$badgeos_submission_content 	= wp_kses_post( $_POST['badgeos_submission_content'], $allowedposttags );
	}

	// Nonce check for security
	check_admin_referer( 'badgeos_submission_form', 'submit_submission' );

    $action =  isset( $_POST['badgeos_submission_submit'] )  ? $badgeos_submission_submit : $badgeos_submission_draft;
...
function badgeos_create_submission( $achievement_id  = 0, $title = '', $content = '', $user_id = 0 , $action = '' ) {
	...
	$post_status = 'publish';
	$post_parent = ( isset( $_POST['post_id'] ) && ( $_POST['post_id'] != 0 ) ) ? absint( sanitize_text_field( $_POST['post_id'] ) ) : "";
    ...
    if( ! empty($_FILES['document_file'] ) && !empty($_FILES['document_file']['name'] ) ) {
        ...
        if( ! isset( $upload['error'] ) && isset($upload['file'] ) ) {
            ...
            $attachment = array(
                ...
                'post_content'      => '',
                'post_status'       => $post_status,
                'post_parent'       => absint( isset( $_POST['post_id'] ) ? sanitize_text_field( $_POST['post_id'] ) : $submission_id )
            );
            ...
            if( $draft_file ) {
                ...
                $attachment['post_parent'] = absint( isset( $_POST['post_id'] ) ? sanitize_text_field( $_POST['post_id'] ) : $submission_id );
                ...
                $attachment['post_parent'] = isset( $_POST['post_id'] ) ? absint( sanitize_text_field( $_POST['post_id'] ) ) : absint( $submission_id );
    ...
    if( isset( $_POST['post_id'] ) && ( sanitize_text_field( $_POST['post_id'] ) != 0 ) ) {
         $new_submission_id = sanitize_text_field( $_POST['post_id'] );
    } else {
...
function badgeos_set_submission_status( $submission_id, $status, $args = array() ) {
	...
	$badgeos_global_settings = get_option( 'badgeos_settings' );
	$global_email_option = $badgeos_global_settings['submission_email'];
	...
	if ( $global_email_option == 'enabled' ) {
...
function badgeos_set_submission_status_submission_approved( $messages, $args ) {
	...
	$email_disable = $badgeos_admin_tools['email_disable_submission_approved_email'];
	$email_subject = $badgeos_admin_tools['email_submission_approved_subject'];
	$email_content = $badgeos_admin_tools['email_submission_approved_content'];
	
	$achievement_name = get_the_title( $args[ 'achievement_id' ] );
	$user_name = $args['user_data']->display_name;
	$submissions_type_url = admin_url('edit.php?post_type=submission');
	$achievement_link = get_permalink( $args[ 'achievement_id' ] );
    ...
    // check if submission approved email disabled
    if( $email_disable == 'yes' ){
    	return $messages;
    }
	...
	if ( $args[ 'auto' ] ) {
		...
		// set email subject
		if( empty( $subject ) ) {
		 	$subject = __( 'Approved Submission: [achievement_name] from [user_name]', 'badgeos' );
		 } 
		
		// set the email message
		if( empty( $message ) ){
			$message = __( 'A new submission has been received and approved:

			In response to: [achievement_name]
			Submitted by: [user_name]

			To view all submissions, including this one, visit: [submission_type_link]', 'badgeos' );
		}
	}
	else {
		...
		if( empty( $subject ) ){
			...
			$subject = __( 'Submission Approved: [achievement_name]', 'badgeos' );
		}

		// set the email message		
		if( empty( $message ) ){

			$message = __( 'Your submission has been approved:

			In response to: [achievement_name]
			Submitted by: [user_name]
			[achievement_link]', 'badgeos' );
		}
	}

	$subject = str_replace('[achievement_name]', $achievement_name, $subject ); 
    ...
    $message = str_replace('[achievement_name]', $achievement_name, $message ); 
    $message = str_replace('[user_name]', $user_name, $message ); 
    $message = str_replace('[submission_type_link]', $submissions_type_url, $message );	
    $message = str_replace('[achievement_link]', $achievement_link, $message );
...
function badgeos_set_submission_status_nomination_approved( $messages, $args ) {
    ...
    $email_disable = $badgeos_admin_tools['email_disable_nomination_approved_email'];
	$email_subject = $badgeos_admin_tools['email_nomination_approved_subject'];
	$email_content = $badgeos_admin_tools['email_nomination_approved_content'];

	$achievement_name = get_the_title( $args[ 'achievement_id' ] );
	$nominee = $args['user_data']->display_name;
	$nominated_by = $args['from_user_data']->display_name;
	$nominations_type_link = admin_url( 'edit.php?post_type=nomination' );
	$achievement_link = get_permalink( $args[ 'achievement_id' ] );
    ...
    // check if nomination approved email disabled
    if( $email_disable == 'yes' ){
    	return $messages;
    }
	...
	if ( $args[ 'auto' ] ) {
		...
		if( empty( $subject ) ){
			...
			$subject = __( 'Approved Nomination: [achievement_name] from [nominee]', 'badgeos' );
		}

		if( empty( $message ) ){

			$message = __( 'A new nomination has been received and auto-approved:

			In response to: [achievement_name]
			Nominee: [nominee]
			Nominated by: [nominated_by]

			To view all nominations, including this one, visit: [nomination_type_link]', 'badgeos' );
		}
		...
		if( empty( $subject ) ){
			...
			$subject = __( 'Nomination Approved: [achievement_name]', 'badgeos' );
		}
		
		if( empty( $message ) ){

			$message = __( 'Your nomination has been approved:
	...
	$subject = str_replace('[achievement_name]', $achievement_name, $subject ); 
    $subject = str_replace('[nominee]', $nominee, $subject );
    $subject = str_replace('[nominated_by]', $nominated_by, $subject );
    ...
    $message = str_replace('[achievement_name]', $achievement_name, $message ); 
    $message = str_replace('[nominee]', $nominee, $message ); 
    $message = str_replace('[nominated_by]', $nominated_by, $message ); 
    $message = str_replace('[nomination_type_link]', $nominations_type_link, $message );	
    $message = str_replace('[achievement_link]', $achievement_link, $message );	
...
function badgeos_set_submission_status_submission_denied( $messages, $args ) {
	...
	$email_disable = $badgeos_admin_tools['email_disable_submission_denied_email'];
	$email_subject = $badgeos_admin_tools['email_submission_denied_subject'];
	$email_content = $badgeos_admin_tools['email_submission_denied_content'];
	...
	if( empty( $subject ) ){
		...
		$subject = __( 'Submission Not Approved: [achievement_name]', 'badgeos' );
	}
	...
	if( empty( $message ) ){
		...
		$message = __( 'Your submission has not been approved:
...
function badgeos_set_submission_status_nomination_denied( $messages, $args ) {
	...
	$email_disable = $badgeos_admin_tools['email_disable_nomination_denied_email'];
	$email_subject = $badgeos_admin_tools['email_nomination_denied_subject'];
	$email_content = $badgeos_admin_tools['email_nomination_denied_content'];
	...
	if( empty( $subject ) ){
		...
		$subject = __( 'Nomination Not Approved: [achievement_name]', 'badgeos' );
	}

	// set the email message
	if( empty( $message ) ) {
		
		$message = __( 'Your nomination has not been approved:
...
function badgeos_set_submission_status_submission_pending( $messages, $args ) {
	...
	$email_disable = $badgeos_admin_tools['email_disable_submission_pending_email'];
	$email_subject = $badgeos_admin_tools['email_submission_pending_subject'];
	$email_content = $badgeos_admin_tools['email_submission_pending_content'];
	
	$achievement_name = get_the_title( $args[ 'achievement_id' ] );
	$user_name = $args[ 'user_data' ]->display_name;
	$submissions_type_url = admin_url( 'edit.php?post_type=submission' );
	$submission_link = admin_url( sprintf( 'post.php?post=%d&action=edit', $args[ 'submission_id' ] ) );
	...
	if( empty( $subject ) ){
		...
		$subject = __( 'Submission: [achievement_name] from [user_name]', 'badgeos' );
	} 
	...
	if( empty( $message ) ){
		...
		$message = __( 'A new submission has been received:
		...
		To view all submissions, visit: [submission_type_link]', 'badgeos' );
	}
    ...
    $message = str_replace('[achievement_name]', $achievement_name, $message ); 
    $message = str_replace('[user_name]', $user_name, $message ); 
    $message = str_replace('[submission_link]', $submission_link, $message );	
    $message = str_replace('[submission_type_link]', $submissions_type_url, $message );
...
function badgeos_set_nomination_status_nomination_pending( $messages, $args ) {
	...
	$email_disable = $badgeos_admin_tools['email_disable_nomination_pending_email'];
	$email_subject = $badgeos_admin_tools['email_nomination_pending_subject'];
	$email_content = $badgeos_admin_tools['email_nomination_pending_content'];

	$achievement_name = get_the_title($args['achievement_id']);
	$nominee = $args['user_data']->display_name;
	$nominated_by = $args['from_user_data']->display_name;
	$nomination_link = admin_url(sprintf('post.php?post=%d&action=edit', $args['submission_id']));
	$nomination_type_link = admin_url('edit.php?post_type=nomination');
	...
	if( empty( $subject ) ){
		...
		$subject = __( 'Nomination: [achievement_name] from [nominee]', 'badgeos' );
	}
	...
	if( empty( $message ) ){
		...
		$message = __( 'A new nomination has been received:
		...
		To view all nominations, visit: [nomination_type_link]', 'badgeos' );
	}
    ...
    $message = str_replace('[nominee]', $nominee, $message ); 
    $message = str_replace('[nominated_by]', $nominated_by, $message );	
    $message = str_replace('[nomination_link]', $nomination_link, $message );
    $message = str_replace('[nomination_type_link]', $nomination_type_link, $message );
...
function badgeos_get_comment_form( $post_id = 0 ) {
	...
	$sub_achievement_id = get_post_meta( $post_id, '_badgeos_submission_achievement_id', true );
	$is_comment_allowed = get_post_meta( $sub_achievement_id, '_badgeos_all_attachment_submission_comment', true );

	// check if comments is allowed by achievements
	if ( $is_comment_allowed != 'on' ) {
		return false;
	}	
    ...
    $sub_form = '<form class="badgeos-comment-form" method="post" enctype="multipart/form-data">';
		...
		// submit button
		$sub_form .= '<p class="badgeos-submission-submit" style="margin-top:10px;"><input type="submit" name="badgeos_comment_submit" value="'. $language['submit'] .'" /></p>'; // <input type="submit" name="badgeos_comment_draft" value="Save Draft" />
		//</p>';
...
function badgeos_save_comment_data() {
	...
	$submission_id  			= isset( $_REQUEST['submission_id'] ) ? sanitize_text_field( $_REQUEST['submission_id'] ) : '';
	...
	$badgeos_comment_draft    	= isset( $_POST['badgeos_comment_draft'] ) ? sanitize_text_field( $_POST['badgeos_comment_draft'] ) : '';
	$badgeos_comment    		= isset( $_POST['badgeos_comment'] ) ? sanitize_textarea_field( $_POST['badgeos_comment'] ) : '';
	$badgeos_comment_nonce    	= isset( $_POST['badgeos_comment_nonce'] ) ? sanitize_text_field( $_POST['badgeos_comment_nonce'] ) : '';
    //Check comment type
    $comment_type = '';
	...
	$comment_data = array(
		...
		'comment_post_ID' 	=> absint( $submission_id ),
		'comment_content' 	=> $badgeos_comment,
    ...
    if( ! empty( $comment_count ) ) {
        ...
        //update comment data
        $where = array( 'comment_ID' => absint( $comment_id ) );
	...
	if ( isset( $badgeos_comment_nonce ) ) {
		global $wp;
		if( empty( $wp->request ) ) {
...
function badgeos_get_nomination_form( $args = array() ) {
	...
	$sub_form = '<form class="badgeos-nomination-form" method="post" enctype="multipart/form-data">';
		...
		$sub_form .= '<p><textarea class="badgeos_nomination_textarea" name="badgeos_nomination_content" id="badgeos_nomination_content-'.absint( $args['achievement_id'] ).'"></textarea></p>';
		$sub_form .= '</fieldset>';
		// submit button
		$sub_form .= '<p class="badgeos-nomination-submit"><input class="badgeos_nomination_btn" id="badgeos_nomination_form" type="submit" name="badgeos_nomination_submit" value="'. esc_attr( $args['submit'] ) .'" /></p>';
		// hidden fields
...
function badgeos_get_submission_form( $args = array() ) {
	...
	$sub_form = '<form class="badgeos-submission-form" method="post" enctype="multipart/form-data">';
		...
		$sub_form .= '<p><textarea placeholder="Type here..." rows="5" name="badgeos_submission_content" id="badgeos_submission_content">' . ( ( $submission_data ) ? $submission_data->post_content : '' ) . '</textarea></p>';
		$sub_form .= '</fieldset>';
		...
		// submit button
		$sub_form .= '<p class="badgeos-submission-submit"><input id="submission_submit_button" type="submit" name="badgeos_submission_submit" value="'. $args['submit'] .'" /> </p>'; //<input type="submit" name="badgeos_submission_draft" value="Save Draft" />
